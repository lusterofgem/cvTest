...
int main()
{
    ...
    // test2: video test
    {
        std::filesystem::path videoPath("test1.mkv");
        ...
        while(true)
        {
            // default
            video >> frame;
            cv::imshow(videoPath.string(), frame);
            std::cout << "default\n";
            int key = cv::waitKey(std::numeric_limits<int>::max());
            if(key == 27)
            {
                return 0;
            }
            ...
            // open
            video >> frame;
            cv::erode(frame, frame, kernel, cv::Point(-1, -1), 10);
            cv::dilate(frame, frame, kernel, cv::Point(-1, -1), 10);
            cv::imshow(videoPath.string(), frame);
            std::cout << "open\n";
            key = cv::waitKey(std::numeric_limits<int>::max());
            if(key == 27)
            {
                return 0;
            }
